  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        borderRadius: '20px', padding: '32px 28px', maxWidth: '440px', width: '100%',
        ...
        <h2 style={{ fontFamily: 'system-ui', fontSize: '22px', fontWeight: 800, color: '#fff', margin: '0 0 20px', letterSpacing: '-0.02em' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px', marginBottom: '20px' }}>
          {[
            ...
            <div key={s.label} style={{ background: GLASS_BG, border: `1px solid ${GLASS_BORDER}`, borderRadius: '12px', padding: '12px', textAlign: 'center' }}>
              <p style={{ fontFamily: 'system-ui', fontSize: '20px', fontWeight: 700, color: '#fff', margin: '0 0 2px' }}>{s.value}</p>
              <p style={{ fontFamily: 'system-ui', fontSize: '10px', color: 'rgba(255,255,255,0.40)', textTransform: 'uppercase', letterSpacing: '0.06em', margin: 0 }}>{s.label}</p>
            </div>
          ))}
        </div>

        <p style={{ fontFamily: 'system-ui', fontSize: '11px', color: 'rgba(255,255,255,0.40)', textTransform: 'uppercase', letterSpacing: '0.10em', margin: '0 0 10px' }}>Round Breakdown</p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginBottom: '24px' }}>
          {rounds.map((r, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 12px', background: GLASS_BG, borderRadius: '10px' }}>
              <span style={{ fontFamily: 'system-ui', fontSize: '11px', fontWeight: 700, color: i % 2 === 0 ? FOCUS_COLOR : BREAK_COLOR, minWidth: '52px' }}>
                {i % 2 === 0 ? 'Focus' : 'Break'} {Math.floor(i / 2) + 1}
              </span>
              <span style={{ fontFamily: 'system-ui', fontSize: '12px', color: '#fff', flex: 1 }}>{r.checkIns} check-ins</span>
              <span style={{ fontFamily: 'system-ui', fontSize: '11px', color: 'rgba(255,255,255,0.45)' }}>
        ...
        <p style={{ fontFamily: 'system-ui', fontSize: '11px', color: 'rgba(255,255,255,0.40)', textTransform: 'uppercase', letterSpacing: '0.10em', margin: '0 0 10px' }}>Avg per round</p>
        <div style={{ background: GLASS_BG, border: `1px solid ${GLASS_BORDER}`, borderRadius: '12px', padding: '14px 16px', marginBottom: '20px' }}>
          <p style={{ fontFamily: 'system-ui', fontSize: '24px', fontWeight: 700, color: '#fff', margin: '0' }}>
            {avgPerRound.toFixed(1)} <span style={{ fontSize: '14px', color: 'rgba(255,255,255,0.45)', fontWeight: 400 }}>check-ins / round</span>
          </p>
        </div>

        <button onClick={onClose} style={{
          width: '100%', background: FOCUS_COLOR, border: 'none', borderRadius: '12px',
          padding: '14px', fontSize: '14px', fontWeight: 700, color: '#000',
...
// ─── Circular Ring ───────────────────────────────────────────────────────────
function CircularRing({ progress, color, size = 140, stroke = 8 }: { progress: number; color: string; size?: number; stroke?: number }) {
...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function PomodoroTimer({ onCheckIn }: { onCheckIn: number }) {
  ...
  const totalRounds = rotations * 2
  const progress = 1 - timeLeft / ((phase === 'focus' ? focusMin : breakMin) * 60 * 1000)
  const ringColor = phase === 'focus' ? FOCUS_COLOR : BREAK_COLOR
  const phaseLabel = phase === 'idle' ? 'Ready' : phase === 'focus' ? 'Focus' : 'Break'
  const roundLabel = phase === 'idle' ? '' : phase === 'focus' ? `Round ${Math.floor(round / 2) + 1}/${rotations}` : `Break ${Math.floor(round / 2) + 1}/${rotations}`

  const cfgBtnStyle: React.CSSProperties = {
    width: '28px', height: '28px', borderRadius: '8px',
    background: 'rgba(255,255,255,0.08)', border: `1px solid ${GLASS_BORDER}`,
    color: '#fff', fontSize: '16px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
  ...
  function SettingRow({ label, value, onChange, min, max }: { label: string; value: number; onChange: (v: number) => void; min: number; max: number }) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
        <span style={{ fontFamily: 'system-ui', fontSize: '13px', color: 'rgba(255,255,255,0.60)' }}>{label}</span>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <button onClick={() => onChange(Math.max(min, value - 1))} style={cfgBtnStyle}>−</button>
          <span style={{ fontFamily: 'system-ui', fontSize: '15px', fontWeight: 700, color: '#fff', minWidth: '28px', textAlign: 'center' }}>{value}</span>
    ...
    <>
      ...
      <div style={{
        ...
        <div onClick={() => setExpanded(e => !e)} style={{
          ...
          <div style={{ flex: 1 }}>
            <p style={{ fontFamily: 'system-ui', fontSize: '13px', fontWeight: 700, color: '#fff', margin: '0' }}>Pomodoro Focus</p>
            {isActive ? (
              <p style={{ fontFamily: 'system-ui', fontSize: '11px', color: ringColor, margin: '0' }}>
                {phaseLabel} · {roundLabel} · {fmt(timeLeft)}
              </p>
            ) : (
              <p style={{ fontFamily: 'system-ui', fontSize: '11px', color: 'rgba(255,255,255,0.35)', margin: '0' }}>
                {focusMin}m focus · {breakMin}m break · {rotations} rounds
              </p>
            )}
          </div>
          <span style={{ fontFamily: 'system-ui', fontSize: '14px', color: 'rgba(255,255,255,0.35)' }}>
            {expanded ? '▲' : '▼'}
          </span>
        </div>
            ...
            {!isActive && (
              <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
                <button onClick={() => setShowSettings(s => !s)} style={{
                  flex: 1, background: GLASS_BG, border: `1px solid ${GLASS_BORDER}`,
                  borderRadius: '10px', padding: '8px', fontSize: '12px', color: 'rgba(255,255,255,0.60)',
                  cursor: 'pointer', fontFamily: 'system-ui',
                }}>
                  {focusMin}m / {breakMin}m / {rotations}×
                </button>
                <button onClick={startSession} style={{
                  flex: 1, background: FOCUS_COLOR, border: 'none',
                  borderRadius: '10px', padding: '8px', fontSize: '13px', fontWeight: 700,
                  color: '#000', cursor: 'pointer', fontFamily: 'system-ui',
                }}>Start</button>
              </div>
            )}

            {showSettings && !isActive && (
              <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: '12px', padding: '14px', marginBottom: '14px' }}>
                ...
                <button onClick={() => { setTimeLeft(focusMin * 60 * 1000); setShowSettings(false) }} style={{
                  width: '100%', background: 'rgba(255,255,255,0.06)', border: `1px solid ${GLASS_BORDER}`,
                  borderRadius: '8px', padding: '8px', fontSize: '12px', color: 'rgba(255,255,255,0.60)',
                  cursor: 'pointer', fontFamily: 'system-ui',
                }}>Apply</button>
              </div>
            )}
            ...
            {isActive && (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}>
                ...
                <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <CircularRing progress={progress} color={ringColor} size={130} stroke={7} />
                  <div style={{ position: 'absolute', textAlign: 'center' }}>
                    <p style={{ fontFamily: 'system-ui', fontSize: '32px', fontWeight: 800, color: '#fff', margin: 0, letterSpacing: '-0.03em' }}>{fmt(timeLeft)}</p>
                    <p style={{ fontFamily: 'system-ui', fontSize: '10px', color: ringColor, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', margin: '2px 0 0' }}>{phaseLabel}</p>
                  </div>
                </div>
                    ...
                    return (
                      <div key={i} style={{ display: 'flex', gap: '3px' }}>
                        <div style={{
                          ...
                          boxShadow: focusActive ? `0 0 6px ${FOCUS_COLOR}` : 'none',
                        }} />
                        <div style={{
                          width: '8px', height: '8px', borderRadius: '50%',
                ...
                {/* Check-in count */}
                <div style={{
                  background: 'rgba(0,0,0,0.3)', border: `1px solid ${GLASS_BORDER}`,
                  borderRadius: '10px', padding: '6px 20px', textAlign: 'center',
                }}>
                  <p style={{ fontFamily: 'system-ui', fontSize: '20px', fontWeight: 800, color: FOCUS_COLOR, margin: 0 }}>{checkIns.length}</p>
                  <p style={{ fontFamily: 'system-ui', fontSize: '9px', color: 'rgba(255,255,255,0.40)', textTransform: 'uppercase', letterSpacing: '0.06em', margin: 0 }}>this session</p>
                </div>
                ...
                <div style={{ display: 'flex', gap: '8px', width: '100%' }}>
                  <button onClick={togglePause} style={{
                    ...
                    color: mode === 'paused' ? FOCUS_COLOR : 'rgba(255,255,255,0.70)',
                    cursor: 'pointer', fontFamily: 'system-ui',
                  }}>
                    {mode === 'paused' ? '▶ Resume' : '⏸ Pause'}
                  </button>
                  <button onClick={extendFive} style={{
                    background: 'rgba(10,186,181,0.12)', border: `1px solid rgba(10,186,181,0.25)`,
