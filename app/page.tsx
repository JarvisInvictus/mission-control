...
function TasksTab() {
  ...
  const milzzyTasks = tasks.filter(t => t.author === "Milzzy");
...
function CheckInsTab({ clients, onClientClick, pomCheckIn }: { clients: Client[]; onClientClick: (c: Client) => void; pomCheckIn: number }) {
  ...
  function StatusBadge({ clientId, status }: { clientId: string; status: CheckInStatus | null }) {
    ...
    const meta = STATUS_META[status] ?? STATUS_META["not-submitted"];
    return (
      <button
...
const P_PROJECT_COLORS: Record<string, string> = {
  teal: "#0abab5",
  amber: "#e8a020",
  blue: "#4a9eff",
  green: "#3dd68c",
...
function P_GlassModal({ children, onClose }: { children: React.ReactNode; onClose: () => void }) {
  return (
    <div
      onClick={onClose}
      style={{
...
function P_ProjectFormModal({
  project,
  onSave,
  onClose,
}: {
  project?: P_Project | null;
  onSave: (data: P_ProjectFormData) => void;
  onClose: () => void;
}) {
  const [form, setForm] = useState<P_ProjectFormData>({
    title: project?.title ?? "",
    desc: project?.desc ?? "",
  ...
  function updateField<K extends keyof P_ProjectFormData>(key: K, value: P_ProjectFormData[K]) {
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            style={{ ...pInputStyle, resize: "vertical", minHeight: "72px" }}
          />
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
              ...
              {COLORS.map(c => (
                <button
                  ...
                  style={{
                    ...
                    background: P_PROJECT_COLORS[c],
                    border: form.color === c ? `2px solid white` : "2px solid transparent",
                    cursor: "pointer",
                    outline: form.color === c ? `2px solid ${P_PROJECT_COLORS[c]}` : "none",
                    outlineOffset: "2px",
                    transition: "all 0.15s",
                    flexShrink: 0,
                  }}
          ...
          <div>
            ...
            <select
              ...
              style={{ ...pInputStyle, cursor: "pointer" }}
            >
              {STATUSES.map(s => (
...
function ProjectsTab() {
  const [projects, setProjects] = useState<P_Project[]>([]);
  const [filter, setFilter] = useState("all");
  const [showModal, setShowModal] = useState(false);
  const [editingProject, setEditingProject] = useState<P_Project | null>(null);
  const [loading, setLoading] = useState(true);
  ...
  async function saveProject(formData: P_ProjectFormData) {
    const payload = {
      ...formData,
  ...
  async function toggleTask(projectId: string, taskId: string) {
    ...
    const saved: P_Project = await res.json();
    setProjects(prev => prev.map(p => p.id === saved.id ? saved : p));
  }

  function openEdit(project: P_Project) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "transparent", padding: "24px 16px 48px" }}>
            ...
            {filtered.length === 0 ? (
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "16px" }}>
                {filtered.map(project => {
                  const accentColor = P_PROJECT_COLORS[project.color] ?? Tiffany;
                  const statusColor = P_STATUS_COLORS[project.status] ?? Tiffany;
      ...
      {/* Modal */}
      {showModal && (
        <P_GlassModal onClose={closeModal}>
          <P_ProjectFormModal
